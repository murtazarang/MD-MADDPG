...
from maddpg.common.mi_gru_cell import MiGRUCell
from maddpg.common.mi_lstm_cell import MiLSTMCell
from maddpg.common.config import config
...
def relu(inp):
    if arglist.relu == "PRM":
        with tf.variable_scope(None, default_name="prelu"):
            ...
            output = pos + neg
    elif arglist.relu == "ELU":
        output = tf.nn.elu(inp)
    # elif config.relu == "SELU":
    #     output = tf.nn.selu(inp)
    elif arglist.relu == "LKY":
        # output = tf.nn.leaky_relu(inp, config.reluAlpha)
        output = tf.maximum(inp, arglist.reluAlpha * inp)
    elif arglist.relu == "STD":  # STD
